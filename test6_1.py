...
def get_weather(lat, lon, start_date, end_date):
    ...
    weather_url = (
        ...
        f"timezone=Asia/Tokyo&start_date={start_date_str}&end_date={end_date_str}"
    )
...
def generate_clothing_suggestions(weather_data, transport):
    if not weather_data:
        return "天気情報を取得できませんでした。"

    prompt = "以下の天気情報に基づいて、適切な服装を提案してください。できるだけ短く、簡潔にしてください。\n\n"
    ...
    prompt += "この情報をもとに、各日に適した服装の提案を書いてください。"

    if transport in ["レンタカー", "マイカー"]:
        prompt += "\n移動手段がレンタカーまたはマイカーであるため、天候による影響を最小限に抑えてアドバイスを追加してください。"
...
def save_map(map_img):
    map_png = map_img._to_png(5)
    image = Image.open(BytesIO(map_png))
    image_path = "map.png"
...
def generate_event_recommendation(user_input, events_data):
    ...
    response = openai.ChatCompletion.create(
        ...
        max_tokens=500,
        temperature=0.7,
        stream=False  # ストリーミングモードを無効にしてみた
    )
...
def main():
    ...
    if "satisfaction" not in st.session_state:
        st.session_state.satisfaction = 1  # default
    if "interest" not in st.session_state:
        st.session_state.interest = 1  # default
    options = ['スケジュールを立てたい', '期間限定イベントに参加したい']
    ...
    if selected_options == 'スケジュールを立てたい':
        ...
        weather_data = get_weather(lat, lon, time1, time2)
        clothing_suggestions = generate_clothing_suggestions(weather_data, how)
        ...
        is_generate_clicked = st.button("あなたにピッタリのプランを見つけ出します")
        ...
        if st.session_state.is_generate_clicked:
            ...
            if selected_options == "スケジュールを立てたい":
                ...
                system_text = (
                    f"あなたは自治体の有能な旅行ガイダンスAIアシスタントで、ミッションはユーザーのニーズに応じて、旅行プランを立てることまたは後述する追加情報を基づいて具体的にユーザーに観光地について教えることです。"
                    f"ユーザーのニーズに応じて答えること。"
            ...
            if st.session_state.map:
                folium_static(st.session_state.map, width=700, height=500)
            st.session_state.message_history.append({"role": "assistant", "content": partial_words})

            if st.session_state.plan:
                st.write(clothing_suggestions)  # 복장 제안 출력
                st.write(st.session_state.plan)  # 여행 일정 출력
            ...
            with st.form(key='plan_form'):
                col1, col2 = st.columns(2)
                with col1:
                    satisfaction = st.radio('このプランに満足していますか？',  stars, index=st.session_state.satisfaction - 1, key='satisfaction_1')
                with col2:
                    interest = st.radio('このプランに沿って行きたいですか？', stars, index=1, key='satisfaction_2')

                submit_button = st.form_submit_button(label='再生成')

                if submit_button:
                    st.session_state.satisfaction = satisfaction
                    st.session_state.interest = interest

        if st.button('この**スペシャルプラン**をpdfに保存します'):
            pdf = PDF()
            pdf.add_page()
            ...
            for line in plan_lines:
                ...
                if len(columns) >= 4:
                    ...
                    max_line = max(len(A1.split('\n')),len(A2.split('\n')),len(A3.split('\n'))                    )

                    cell_hgt = 10*max_line
                    pdf.cell(30, cell_hgt, A1, border=1, align='C')
                    pdf.cell(80, cell_hgt, A2, border=1, align='C')
                    pdf.multi_cell(80, cell_hgt, A3, border=1, align='L')

            pdf.add_page()
            if st.session_state.map:
                map_path = save_map(st.session_state.map)
                pdf.image(map_path, x=10, y=60, w=180)
